...
- Your instructor has provisioned a Rocky Linux 9 VM for this lab, `lab07-patch01`
...
Patch management is where "apply the update" meets "don't break production" - a team that patches blind, with no pre/post vulnerability comparison and no tested rollback path, is one bad kernel update away from an outage nobody can quickly undo. This lab treats patching as a full lifecycle: inventory what's vulnerable, patch deliberately, verify the patch actually closed what it claimed to, and prove - with a filesystem integrity baseline via AIDE - that nothing changed beyond what the patch was supposed to touch.
...
Write a formal rollback procedure (`lab07-rollback.md`) that a different engineer could follow if a patch causes a service outage. The procedure must cover:
...
- `lab07-rollback.md` - rollback procedure
...
| Item | Points |
|------|--------|
| Vulnerability inventory - scan + CVSS-prioritized table (Part 1) | 15 |
| `dnf-automatic` configured and annotated (Part 2) | 12 |
| AIDE baseline initialized before patching (Part 3) | 8 |
| Patches applied and verified (Part 4) | 15 |
| AIDE post-patch check - every finding classified (Part 5) | 15 |
| Rescan, CVE delta table, exception documentation (Part 6) | 20 |
